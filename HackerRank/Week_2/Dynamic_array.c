...
int **seqs = malloc(n * sizeof(*seqs));
    int *len = calloc(n, sizeof(int));
    int *cap = calloc(n, sizeof(int));
    for (int i = 0; i < n; i++) seqs[i] = NULL;
    ...
    *result_count = 0;
    ...
    for (int q = 0; q < queries_rows; q++) {
        ...
        if (t == 1) {
            if (len[idx] == cap[idx]) {
                int newCap = (cap[idx] == 0) ? 2 : cap[idx] * 2;
                int *newSeq = realloc(seqs[idx], newCap * sizeof(int));
                seqs[idx] = newSeq;
                cap[idx] = newCap;
            }
            seqs[idx][len[idx]++] = y;
            ...
            int pos = y % len[idx];
            lastVal = seqs[idx][pos];
            answers = realloc(answers, (*result_count + 1) * sizeof(int));
            answers[*result_count] = lastVal;
            (*result_count)++;
        }
    }
